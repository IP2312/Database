...
SELECT LPAD('-', (LEVEL - 1) * 3) || SUB, LEVEL
from Parts
...
SELECT LPAD('-', (LEVEL - 1) * 3) || ENAME
from EMP
Start with EMPNO = (Select EMPNO from emp where ENAME = 'SMITH')
connect by prior MGR = EMPNO;

SELECT LEVEL, round(avg(sal), 2) as average_salary
from EMP
Start WITH EMPNO = (SELECT EMPNO from emp Where JOB = 'PRESIDENT')
connect by prior EMPNO = MGR
group by LEVEL;
SELECT last_updated from ACCOUNTS;